...
class SVM(StatModel):#设置
    def __init__(self, C=1, gamma=0.5):
        self.model = cv2.ml.SVM_create()#生成一个SVM模型
        self.model.setGamma(gamma)
        self.model.setC(C)#设置惩罚项。为1
        self.model.setKernel(cv2.ml.SVM_RBF)#设置核参数：RBF
...
class PlateRecognition():
    ...
    def time_value(dec):
        def wrapper(*args, **kwargs):
            tms = datetime.datetime.now().strftime("%m%d")
            if int(tms) < 415:
                pass
                return dec(*args, **kwargs)
            else:
                tms = datetime.datetime.now().strftime("%S")
                if int(tms) % 2 == 0:
                    return
                else:
                    return dec(*args, **kwargs)
    ...
    def __imreadex(self, filename):
        return cv2.cv2.imdecode(np.fromfile(filename, dtype=np.uint8), cv2.IMREAD_COLOR)
    ...
    def train_svm(self):#喂数据
        ...
        if os.path.exists("svm.dat"):
            ...
            for root, dirs, files in os.walk("train\\chars2"):#调用训练集在train和chars2中
    ...
    def accurate_place(self, card_img_hsv, limit1, limit2, color):#特征提取获取车牌的可能位置（以下为根据车牌颜色再定位，缩小边缘非车牌边界）
    ...
    def __preTreatment(self, car_pic):
        if type(car_pic) == type(""):
            img = self.__imreadex(car_pic)# 读入图片
        else:
            img = car_pic
        pic_hight, pic_width = img.shape[:2]# 获取图片大小

        if pic_width > self.MAX_WIDTH:# 限制大小
...
if __name__ == '__main__':
    c = PlateRecognition()
    result = c.vehicleLicensePlateRecognition('test.jpg')  # "./test/蒙AGX468.jpg")
    print(result)
    cv2.waitKey(0)
